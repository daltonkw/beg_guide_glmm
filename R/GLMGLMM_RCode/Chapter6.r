...
model{
  ...
  #1A. Diffuse normal priors for regression coefficients
  for (i in 1:K) { beta[i] ~ dnorm(0, 0.0001) }

  #1B. Diffuse gamma prior for variance term
  r ~ dgamma(0.01, 0.01 )
...
#Estimate the scale using the formula from Venables and Ripley (2002)
library(MASS)
...
# To present a prediction interval, add te following code to JAGS:
...
Nre <- length(unique(Seedlings$Species))
...
cat("
model{
  #1A. Priors for regression coefficients
  for (i in 1:K) { beta[i] ~ dnorm(0, 0.0001) }

  #1B. Priors for random intercepts and random slopes
  for (i in 1:Nre) {
    a[i] ~ dnorm(0, tau.ri)
    b[i] ~ dnorm(0, tau.rs) 
  }

  #1C. Prior for term in gamma variance
  tau      ~ dgamma( 0.01, 0.01 )

  #1D. Priors for variances random intercepts/slopes
...
model{
  ...
  #1B. half-Cauchy(25) prior for sigma
  num   ~ dnorm(0, 0.0016)
  denom ~ dnorm(0, 1)
  sigma <- abs(num / denom)
  tau <- 1 / (sigma * sigma)

...
AllY  <- as.vector(as.matrix(ZooData[,MyVar]))
AllX  <- rep(1:nrow(ZooData), length(MyVar))
AllID <- rep(MyVar, each = nrow(ZooData))
...
#Figure 7.5
par(mar = c(5,5,2,2))
plot(resid(M2) ~ fitted(M2, type = "response"), 
     xlab = "Fitted values",
     ylab = "Pearson residuals",
     cex.lab = 1.5)
abline(h = 0, lty = 2)
...
cat("
model{
    #1A. Diffuse normal priors for the betas
    for (i in 1:K) { beta[i] ~ dnorm(0, 0.0001) }
      
    #1B. Normal priors for the random intercepts
...
cat("
model{
    #1A. Priors beta and random effects
    for (i in 1:K) { beta[i] ~ dnorm(0, 0.0001) }  
    for (i in 1:Nre) { a[i] ~ dnorm(0, tau) } 

    #1B. half-Cauchy(25) prior sigma zoo
    num   ~ dnorm(0, 0.0016) 
    denom ~ dnorm(0, 1)
    sigma <- abs(num / denom) 
    ...
    for (i in 1:N){  
      ...
      ExpY[i] <- p[i] * Scans[i]
      VarY[i] <- p[i] * Scans[i] * (1 - p[i]) * (1 + (Scans[i] -1) / (theta + 1))
      PRes[i] <- (Y[i] - ExpY[i]) / sqrt(VarY[i])
      YNew[i]   ~ dbin(p[i], Scans[i])   #New data
      PResNew[i] <- (YNew[i] - ExpY[i]) / sqrt(VarY[i])
      D[i]       <- pow(PRes[i], 2)
...
J1 <- update(J0, n.iter=50000, n.thin = 10)  
...
# Have a look at: 
# http://www.sciencedirect.com/science/article/pii/S0304380014001690
# For a proper implementation of the stuff below.

Zoo <- as.numeric(ZD$fZoo)
Nre <- length(unique(Zoo))
...
model{
    ...
    #Priors for random effects and sigma zoo
    for (i in 1:Nre){ a[i] ~ dnorm(0,tau.zoo) }

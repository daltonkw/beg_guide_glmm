...
#Code used to create the graph in the second print of the book

AllY  <- as.vector(as.matrix(SQ[,MyVar]))
AllX  <- rep(1:nrow(SQ), length(MyVar))
AllID <- rep(MyVar, each = nrow(SQ))
...
K1 <- jags(data       = win.data, 
           ...
           n.burnin   = 4000)
K2 <- update(K1, n.iter = 10000, n.thin = 10)
mean(K2$BUGSoutput$sims.list$Fit >  
     K2$BUGSoutput$sims.list$FitNew) 
